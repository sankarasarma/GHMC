...
public class FileProcessor {
	...
	private static final SimpleDateFormat OLD_DATE_FORMAT = new SimpleDateFormat("MM/dd/yyyy");
	private static final SimpleDateFormat NEW_DATE_FORMAT = new SimpleDateFormat("dd/MM/yyyy");
	

	private static List<File> fetchFilesFromResource() {
		File resourceFiles = new File("src/main/resource");
		ArrayList<File> files = new ArrayList<File>(Arrays.asList(resourceFiles.listFiles()));
		int index = 0;
		for (File file : files) {
			if(file.isDirectory()) {
				break;
	...
	private static boolean isValidRow(XSSFRow aRow) {
		try {
			Iterator<Cell> cells = aRow.cellIterator();
			XSSFCell aCell = null;
			while(cells.hasNext()) {
				aCell = (XSSFCell) cells.next();
				String slNo = aCell.getStringCellValue();
	...
	public static void readFile() throws IOException {
		...
		List<File> files = fetchFilesFromResource();
		...
		for (File aFile : files) {
			...
			try {
				...
				Iterator<Sheet> sheets = aWorkbook.iterator();
				while (sheets.hasNext()) {
					aSheet = (XSSFSheet) sheets.next();	
					String sheetName = aSheet.getSheetName();
					String newSheetName = fileName+"_"+sheetName;
					System.out.println("New Sheet Name --> "+newSheetName);
					Iterator<Row> rows = aSheet.rowIterator();
					while (rows.hasNext()) {
						aRow = (XSSFRow) rows.next();
						if(isValidRow(aRow)) {
